...
### Localizzazione
L'interfaccia esiste in sei lingue: italiano, inglese, tedesco, spagnolo, francese e svedese, un `lib/l10n/app_<lingua>.arb` ciascuna. `app_it.arb` è il **template**: è l'unico che porta i blocchi `@chiave` con le descrizioni, ed è da lì che nasce ogni chiave nuova. Le altre devono avere le stesse chiavi con gli stessi placeholder e nello stesso ordine — una chiave che manca non è un errore di compilazione, semplicemente a runtime ricade sull'italiano. Il test di parità (`test/l10n/arb_parity_test.dart`) **scopre i file dal disco**, non da un elenco scritto a mano: un elenco a mano dimentica esattamente la lingua appena aggiunta.

**Di default si segue il telefono**, e da *Impostazioni → Lingua* si può forzarne una (`LocaleCubit`, preferenza `ui.locale` nel secure storage). Lo stato del cubit è una `Locale?` e `null` significa "come il sistema": è esattamente ciò che `MaterialApp.locale` vuole, quindi si passa così com'è. Il cubit sta nella composition root perché il suo lettore è `MaterialApp`, che il router se lo costruisce dentro. I nomi delle lingue nel selettore sono **endonimi** e stanno in `core/l10n/language_names.dart`, non negli ARB: "Deutsch" si scrive così anche in un'interfaccia italiana, e un elenco tradotto sarebbe illeggibile proprio a chi lo apre per uscire da una lingua che non capisce.

Regole che mordono:
- **il ripiego è esplicito** (`AppConstants.fallbackLocale`, usato da `App.resolveLocale`). Quello di Flutter è il primo elemento di `supportedLocales`, e la lista generata è in ordine alfabetico: senza, un telefono in giapponese finirebbe in tedesco. Anche la lingua scelta a mano passa da lì. Per lo stesso motivo i widget test che cercano stringhe italiane devono fissare `locale: const Locale('it')` sul loro `MaterialApp`;
...
- **i prompt sono in inglese, il contenuto delle schede no**: tutto ciò che parla al modello — `services/ai/prompts.dart`, le `description` dello schema JSON, i messaggi di validazione e le eccezioni di `plan_parser.dart` — è in inglese, perché è la lingua in cui i modelli seguono meglio le istruzioni e perché ormai l'interfaccia non ne ha una sola. Ciò che il modello *produce* deve invece restare nella lingua della scheda: nomi di esercizi, etichette dei giorni, titoli di sezione e note. È la regola che un prompt inglese mette più a rischio (il modello tende a rispondere nella lingua in cui gli si parla), quindi in `_extractionRules` ha una sezione sua, "Language of the answer", ripetuta nel prompt di trascrizione, nei due messaggi correttivi e nelle `description` dello schema. L'esempio JSON dentro il prompt è apposta in italiano con i nomi dei campi in inglese: mostra la separazione invece di descriverla. Solo i messaggi del parser affiorano anche all'utente (dentro `aiPasteParseFailed`), e restano inglesi perché la loro ragione d'essere è tornare al modello nel retry;
...
- `lib/core/design/` — `AppPalette`, `AppTypography`, `AppChrome` (ombre + barre di sistema), `AppRadius`, `AppSpacing`, `theme_context.dart` e `linear_icons.dart`.

**Due temi, un impaginato solo.** Il file di design disegna il chiaro; lo scuro è lo stesso impaginato con la scala di luminosità rovesciata (pagina più scura, card un gradino sopra, blocco pieno un gradino sopra ancora), e lo costruisce la **stessa** `AppTheme._build(brightness)`: una differenza fra i due che non sia un colore è un bug. Come per la lingua, di default si segue il telefono e da *Impostazioni → Tema* si può forzarne uno (`ThemeModeCubit`, preferenza `ui.themeMode` nel secure storage, `ThemeMode.system` = "come il sistema"); il cubit sta nella composition root accanto a `LocaleCubit` perché a leggerlo è `MaterialApp`.

**I token si leggono dal contesto**, non più per nome: `context.colors.ink`, `context.type.rowStrong`, `context.chrome.floating` (estensione in `core/design/theme_context.dart`, che si aggancia a `Theme.of(context).brightness`). `AppRadius` e `AppSpacing` restano costanti statiche: un raggio non cambia col tema, e passare anche quelli dal contesto nasconderebbe cosa dipende davvero dalla luminosità. Un default di costruttore non può leggere il contesto: il parametro diventa `Color?` e si risolve nel `build` (`foreground ?? context.colors.ink`), come fanno `SquareIconButton` e `SurfaceCard`.

**Palette.** Nel chiaro: fondo `#F4F4F6`; superfici bianche raggio 26; inchiostro `#192126`; prosa `#232A3A`; secondario `#8C9092`; contorni `#D4D8E0`. Regola non negoziabile: **un solo elemento lime `#BBF246` per schermata** (la scheda in uso, l'esercizio corrente, la tab attiva). È l'intero meccanismo con cui si legge "cosa è vivo adesso": il secondo lime annulla il primo. Il rosa `#FF5678` è il distruttivo e compare come *testo* (voci di menu, "Rimuovi serie") ovunque tranne che nella conferma finale di eliminazione, l'unico punto in cui diventa un fondo.

**I ruoli che il tema scuro può rompere in silenzio** — sono due, e stanno nei tipi apposta:
- **lime e rosa non si capovolgono**: sono già chiari e valgono identici nei due temi, quindi ciò che ci sta sopra deve restare scuro. `context.colors.ink` lì è sbagliato (al buio è quasi bianco): si usano `onLime` / `onDanger`, e `onLimeSurface` per una superficie chiara appoggiata *sul* lime (il disco della spunta sulla scheda in uso);
- **`ink` è il testo, `inkSurface` è il blocco pieno** (pillola principale, tab bar, barra del timer, snackbar), con `onInkSurface` sopra. Nel chiaro coincidono, ma al buio `ink` si schiarisce e `inkSurface` **no**: tab bar e barra del timer si portano dentro il lime, e il lime su un blocco quasi bianco sparisce (1,1:1).

Aggiungere un colore vuol dire aggiungere un ruolo a **entrambe** le palette in `app_palette.dart`. `test/core/design/app_palette_test.dart` verifica che ogni coppia testo/fondo dello scuro passi WCAG AA e che le due tipografie differiscano solo nei colori; `on_lime_surfaces_test.dart` ridisegna le superfici lime nei due temi e controlla i colori.
...
- **il widget esportato deve bastare a sé stesso**: niente `Expanded` e niente scrollable (in altezza non c'è nessun viewport da riempire), e si porta addosso `Directionality`, `MediaQuery`, `Localizations` e `Theme`, perché sopra di lui non c'è nessun `MaterialApp`. Lingua **e** luminosità gliele passa il chiamante (`locale`, `brightness`) per la stessa ragione: non c'è nessuno da cui ereditarle. Si condivide il tema in cui si sta guardando la scheda. Il `MediaQuery` è quello di default apposta: il fattore di ingrandimento del testo di chi condivide non deve sfondare un'impaginazione a larghezza fissa;
...
- un widget test che non passa `theme:` gira sul tema chiaro di Flutter, e i token dal contesto ripiegano sulla palette chiara: va bene per tutto ciò che non parla di colori. Per verificare il tema scuro si passa `theme: AppTheme.of(brightness)` (o `theme:`/`darkTheme:`/`themeMode:` insieme, se serve il cambio a caldo, come in `settings_page_test.dart`).
