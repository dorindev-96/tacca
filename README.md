...
> **Language note.** The source comments and the specification are in Italian (*"tacca"* is the tally mark you scratch down after a set). This README and the contributor docs are in English. The UI ships in six languages — Italian, English, German, Spanish, French and Swedish — one `lib/l10n/app_<lang>.arb` each, with `app_it.arb` as the template that carries the descriptions. The app follows the phone's language, falls back to Italian when it is none of the six, and *Settings → Language* overrides both. Adding a seventh is a translation, not a refactor: drop in the ARB file, add its name to `core/l10n/language_names.dart`, and `gen-l10n` picks it up. The AI prompts are a separate matter: they are written in English, and they instruct the model to keep the plan itself in whatever language it was written in.
...
- **One visual language, two readings of it.** The look comes from a design file, not from a Material seed colour: a single lime accent `#BBF246` reserved for the one live thing on screen (the plan in use, the current exercise, the active tab), surfaces at radius 26, Lato for the interface and the platform font for prose. The design file draws the light theme; the dark one is the same layout with the luminance ladder inverted — page darkest, cards a step above it, the filled block a step above those — built by the *same* function, so a difference between the two that is not a colour is a bug. The app follows the phone and *Settings → Theme* overrides it, exactly like the language.
- **Lime and pink do not invert, so neither does what sits on them.** They are already light colours and are identical in both themes, which makes `ink` (near-white in the dark theme) the wrong colour for text on a lime pill. `AppPalette` therefore names those roles separately — `onLime`, `onDanger`, `onLimeSurface` — and splits `ink` (text) from `inkSurface` (the filled block: primary pill, tab bar, timer bar, snackbar), which in the dark theme must stay dark so the lime it carries still reads. It is the one design rule a dark theme could have broken silently, so it is in the types and in the tests rather than in a comment.
